...
test("a function graph carries its curves, crossing, marks and slope triangle", () => {
  ...
  assert.equal(spec.expressions[0].latex, "y=2x-1");
  assert.ok(spec.markers.some((m) => m.label === "(2, 3)"), JSON.stringify(spec.markers));
  assert.ok(spec.expressions.some((e) => e.lineStyle === "DASHED" && e.latex.startsWith("y=1\\left\\{")));
  assert.ok(spec.expressions.some((e) => e.label === "rise 4"));
  // Slope labels ride on near-zero dots, never on invisible ones (that hides the text).
  const slopeLabels = spec.expressions.filter((e) => e.label === "rise 4" || e.label === "run 2");
...
test("joined points become a polygon, and empty axes are just a view", () => {
  ...
  assert.ok(spec.expressions[0].latex.startsWith("\\operatorname{polygon}((1,1),(4,1),(4,3))"), spec.expressions[0].latex);
